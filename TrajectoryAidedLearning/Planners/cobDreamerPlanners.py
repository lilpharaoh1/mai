import numpy as np 
from TrajectoryAidedLearning.Utils.cobdreamer.dreamerv3 import DreamerV3
...
class cobDreamerTrainer: 
    ...
    def plan(self, obs, context=None, add_mem_entry=True):
        ...
        self.nn_act, self.nn_rssm, self.nn_mask = self.agent.act(self.nn_state, self.nn_act, self.nn_rssm, context=self.nn_context, is_first=True if self.nn_act is None else False)
    ...
    def add_memory_entry(self, obs, done=False):
        if self.nn_state is not None:
            ...
            transition['mask'] = np.array(self.nn_mask)
