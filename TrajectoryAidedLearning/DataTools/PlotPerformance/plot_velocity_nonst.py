...
class AnalyseTestLapData:
        ...
        for run_num, run_folder in enumerate(run_folders):
            ...
            for idx, folder in enumerate(run_folder):
                try:
                    ...
                    self.plot_succ_mat(indv_succ, save_path=f'{folder}velocity_{idx}')
                except:
                    print(f"NOTE -> not counting folder {folder}")
            self.plot_succ_mat(np.mean(run_succ, axis=0).reshape(1, 7, 7), save_path=f'{run_names[run_num]}velocity')
    ...
    def plot_succ_mat(self, data, save_path=None):
        ...
        cax = ax.matshow(data, cmap='magma_r', vmin=0.0, vmax=6.0)
        ...
        for i in range(data.shape[0]):
            for j in range(data.shape[1]):
                ax.text(j, i, f'{data[i, j]:.2f}', va='center', ha='center', color='black' if data[i, j] < 3.0 else 'white')
    ...
    def find_succ(self, folder):
        ...
        for i in range(0, 49):
            ...
            with open(yaml_path) as file:
                ...
                indv_succ[i // 7, i % 7] = run_config['avg_velocity']
