        ...
        for run_num, run_folder in enumerate(run_folders):
            ...
            for idx, folder in enumerate(run_folder):
                try:
                    ...
                    self.plot_succ_mat(indv_succ, save_path=f'{folder}step_reward_{idx}')
                except:
                    print(f"NOTE -> not counting folder {folder}")
            self.plot_succ_mat(np.mean(run_succ, axis=0).reshape(1, 7, 7), save_path=f'{run_names[run_num]}step_reward')
    ...
    def find_succ(self, folder):
        ...
        for i in range(0, 49):
            ...
            with open(yaml_path) as file:
                ...
                indv_succ[i // 7, i % 7] = run_config['avg_step_reward']
