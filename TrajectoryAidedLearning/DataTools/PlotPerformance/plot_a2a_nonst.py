...
class AnalyseTestLapData:
    ...
    def plot_succ_mat(self, data, save_path=None):
        ...
        cax = ax.matshow(data, cmap='magma_r', vmin=0.0, vmax=50.0)
        ...
        for i in range(data.shape[0]):
            for j in range(data.shape[1]):
                ax.text(j, i, f'{data[i, j]:.2f}', va='center', ha='center', color='black' if data[i, j] < 25.0 else 'white')
    ...
    def find_succ(self, folder):
        ...
        for i in range(0, 49):
            ...
            with open(yaml_path) as file:
                ...
                indv_succ[i // 7, i % 7] = run_config['a2a_collisions']
