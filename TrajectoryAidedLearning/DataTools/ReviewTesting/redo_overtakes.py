...
class AnalyseTestLapData:
    ...
    def load_testing_stats(self, folder, run):
        ...
        for lap_n in range(50):
            ...
            overtakes.append(self.num_agents - places[0] - 1)
        
        overtakes = np.array(overtakes)
    ...
    def edit_run(self, folder, run, data):
        ...
        run_config['avg_overtakes'] = float(np.mean(data))
        run_config['overtakes_std_dev'] = float(np.std(data))
