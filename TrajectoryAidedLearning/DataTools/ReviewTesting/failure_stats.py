...
class AnalyseTestLapData:
    ...
    def explore_folder(self, run_file):
        ...
        iid_total = 50 * 9 * 5
        ood_total = 50 * 40 * 5
        print("total iid & ood:", iid_total, ood_total)
        print("-----------------------------------------------")
        for run_num, run_folder in enumerate(run_folders):
            ...
            for idx, folder in enumerate(run_folder):
                ...
                for run in range(0, 49):
                    ...
                    success_counter, a2a_counter = self.load_testing_stats(folder, run)
                    success_dist[run // 7, run % 7] += success_counter
                    a2a_dist[run // 7, run % 7] += a2a_counter

            iid_a2a = np.sum(a2a_dist[in_dist].reshape(-1))
            iid_success = np.sum(success_dist[in_dist].reshape(-1))
            ood_a2a = np.sum(a2a_dist[~in_dist].reshape(-1))
            ood_success = np.sum(success_dist[~in_dist].reshape(-1))

            print(f"In-distribution) Complete Laps: {iid_success} / A2A Collisions : {iid_a2a} / Track Collisions : {iid_total - iid_success - iid_a2a}")
            print(f"Out-of-distribution) Complete Laps: {ood_success} / A2A Collisions : {ood_a2a} / Track Collisions : {ood_total - ood_success - ood_a2a}")
    ...
    def load_testing_stats(self, folder, run):
        ...
        a2a_counter = run_config['a2a_collisions']

        # print("Success counter:", success_counter)
        # print("Place counter:", place_counter)

        return success_counter, a2a_counter
