...
# from TrajectoryAidedLearning.Utils.director.replay_buffer import ReplayBuffer
from TrajectoryAidedLearning.Utils.director.config import ExperimentConfig, MultiAgent
from TrajectoryAidedLearning.Utils.director.policy import Manager, Worker
from TrajectoryAidedLearning.Utils.director.goal_autoencoder import GoalAutoencoder
from TrajectoryAidedLearning.Utils.director.utils import (
...
class Director(object):
    def __init__(self, state_dim, action_dim, name, max_action=1, window_in=1, window_out=1, multiagent=True, device=None):
        ...
        self.goal_update_idxs = 0
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu") if device is None else device
        print("self.device :", self.device)

        self.cfg = MultiAgent()
        self.cfg.rssm_type = "continuous"
        config = self.cfg

        self.goal_vae_latents, self.goal_vae_classes = 8, 8
        self.goal_duration = 4
        self.onpolicy_goal_iteration = 0
    ...
    def act(self, state, prev_action, prev_rssm):
        ...
        with torch.no_grad():
            embed = self.ObsEncoder(torch.FloatTensor(state)).unsqueeze(0)
            ...
            if (self.onpolicy_goal_iteration % self.goal_duration) == 0:
                env_goal, _ = self.manager.get_action(model_state)
                env_goal, _ = self.GoalVAE.decode(env_goal)
                self.onpolicy_goal = env_goal
                self.onpolicy_goal_iteration = 1
            action, _ = self.worker.get_action(model_state, self.onpolicy_goal)
    ...
    def train(self):
        ...
        for i in range(self.collect_intervals):
            ...
            _, posterior = self.RSSM.rollout_observation(self.seq_len, self.ObsEncoder(obs), actions, nonterms, self.RSSM._init_rssm_state(self.batch_size))
            ...
            goal_reconstruction, reconstruction_dist = self.GoalVAE.decode(goal_embedding)
            goal_losses = self.GoalVAE.loss(post_modelstate, reconstruction_dist, goal_logit)
            goal_loss = goal_losses["loss"]

            self.goal_vae_optimizer.zero_grad()
            goal_loss.backward()
            torch.nn.utils.clip_grad_norm_(self.GoalVAE.parameters(), self.grad_clip_norm)
            self.goal_vae_optimizer.step()
            _goal_vae_loss.append(goal_loss.detach().cpu().item())
            
            # Loss for Manager & Worker
            (
                worker_actor_loss,
                worker_value_loss,
                manager_actor_loss,
    ...
    def rollout_imagination(
        ...
        def _get_goal(model_state):
            ...
            entropy = goal_dist.entropy() # .sum(dim=-1) # EMRAN Hope this doesn't break anything :/ # EMRAN Appears to have fixed everything, funny that
    ...
    def director_loss(self, posterior):
        ...
        manager_modelstates = torch.cat([self.RSSM.get_model_state(batched_posterior).unsqueeze(0), imag_modelstates[:-1]], dim=0)[::self.goal_duration]
        ...
        with FreezeParameters(self.world_list + self.value_list + [self.DiscountModel]):
            ...
            worker_value = self.worker.critic.target_value(torch.cat([imag_modelstates[:-1], goals[1:]], dim=-1)).mean
            ...
            reconstructed_modelstates, _ = self.GoalVAE.decode(self.GoalVAE.encode(imag_modelstates)[0])
            reconstruction_loss = torch.mean((reconstructed_modelstates - imag_modelstates) ** 2, dim=-1, keepdim=True)
    ...
    def create_agent(self):
        config = self.config
        obs_shape = (1, self.state_dim) # config.obs_shape
        action_size = self.act_dim # config.action_size
