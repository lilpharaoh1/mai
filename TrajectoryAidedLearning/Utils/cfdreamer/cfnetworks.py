...
class RSSM(nn.Module):
    ...
    def img_step(self, prev_state, prev_action, dcontext=None, sample=True):
        ...
        if not dcontext is None and self._add_dcontext:
            x = torch.concatenate([x, dcontext], -1)
        # (batch, stoch * discrete_num + action + context, embed) -> (batch, hidden)
        ...
        if not dcontext is None and self._add_dcontext_posterior:
            x = np.concatenate([x, dcontext], -1)
...
class MultiEncoder(nn.Module):
    def __init__(
        ...
        excluded = ("is_first", "is_last", "is_terminal", "reward", "context")
        shapes = {
            k: v
...
class MultiDecoder(nn.Module):
    def __init__(
        ...
        # self.mlp_shapes = shapes
        # self.cnn_shapes = None
        excluded = ("is_first", "is_last", "is_terminal", "context")
